...
def make_tiles(img, mask, IMG_SIZE):
    '''
...
def create_modified_dataset(csv_path, IMAGES_DIR, MOD_IMAGES_DIR, MOD_MASKS_DIR, IMG_SIZE):
    '''
    ...
    for img_name in (images_names):
        ...
        for i in range(len(imgs)):
            cv2.imwrite(MOD_IMAGES_DIR+f"/{id}.png", imgs[i]*255)
            cv2.imwrite(MOD_MASKS_DIR+f"/{id}.png", masks[i]*255)
